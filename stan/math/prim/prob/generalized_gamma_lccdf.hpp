#ifndef STAN_MATH_PRIM_PROB_GENERALIZED_GAMMA_LCCDF_HPP
#define STAN_MATH_PRIM_PROB_GENERALIZED_GAMMA_LCCDF_HPP
...
return_type_t<T_y, T_loc, T_scale, T_shape> generalized_gamma_lccdf(
  ...
  static constexpr const char* function = "generalized_gamma_lccdf";
  ...
  for (size_t n = 0; n < N; ++n) {
    ...
    if (Q_dbl == 0.0) {
      ...
      log_ccdf += LOG_HALF + std::log(erfc_val);
      ...
      if constexpr (is_any_autodiff_v<T_y, T_loc, T_scale, T_shape>) {
        ...
        if constexpr (is_autodiff_v<T_shape>) {
          ...
          auto logccdf_eval = [&](double q_val) {
          ...
          const double deriv
              = (logccdf_eval(eps) - logccdf_eval(-eps)) / (2.0 * eps);
    ...
    double ccdf_val = 0.0;
    if (Q_dbl > 0.0) {
      ccdf_val = gamma_q(inv_sq, z);
    } else {
      ccdf_val = gamma_p(inv_sq, z);
    }

    log_ccdf += std::log(ccdf_val);
    ...
    if constexpr (is_any_autodiff_v<T_y, T_loc, T_scale, T_shape>) {
      ...
      if constexpr (is_autodiff_v<T_y>) {
        partials<0>(ops_partials)[n] += -pdf / ccdf_val;
      }
      if constexpr (is_autodiff_v<T_loc>) {
        const double dccdf = -sign_Q * gamma_pdf * dz_dmu;
        partials<1>(ops_partials)[n] += dccdf / ccdf_val;
      }
      if constexpr (is_autodiff_v<T_scale>) {
        const double dccdf = -sign_Q * gamma_pdf * dz_dsigma;
        partials<2>(ops_partials)[n] += dccdf / ccdf_val;
      }
      if constexpr (is_autodiff_v<T_shape>) {
        const double dccdf
            = -sign_Q * (gamma_pdf * dz_dQ + grad_reg * dk_dQ);
        partials<3>(ops_partials)[n] += dccdf / ccdf_val;
      }
    }
  }
